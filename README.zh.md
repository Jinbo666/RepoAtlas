...
框架大部分是静默运行的。以下是你实际需要做的（以及不需要做的）：

### Session 开始 — 你什么都不用说

Cursor、Claude Code、Codex、Windsurf 等工具在会话开始时**自动读取 `AGENTS.md`**。agent 会自己按路由链加载上下文（`current-focus.md` → `active.md` → `system-overview.md`）。你直接开始工作即可。

如果上下文快满了、或者 agent 看起来「迷路了」，一句话重置：

> 重新读一下 AGENTS.md，对齐当前状态。

### 工作过程中 — 读取自动，写入需要你提一句

agent 擅长**读取**记忆（加载上下文、查看错题本），但很少**主动写回**——更新日志、教训、焦点对模型来说像是"额外工作"。

**不需要每次都提醒，只在关键节点说一句：**

| 时机 | 说什么 |
|------|--------|
| 修了一个有教训价值的 bug | *「把这个记到 recent-lessons 和 log 里。」* |
...
**读取是隐式的，写入只需你说一句。** 随着模型能力提升，写入也会变得自动——协议已经定义好了，等着被执行。
